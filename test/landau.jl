...
function calcall(∇²f::SparseMatrixCSC, ∇f::Vector{T}, dofvector::Vector{T}, model::LandauModel{T}) where T
    ...
    @assemble! begin
        ...
        ForwardDiff.hessian!(cache.element_hessian, cellcache.elpotential, eldofs, cache.hessconf)
        ForwardDiff.gradient!(cache.element_gradient, cellcache.elpotential, eldofs, cache.gradconf)
        @inbounds assemble!(assemblers[threadid()], cache.dofindices, cache.element_gradient, cache.element_hessian)
    end
    return sum(outs)
end
# ## Minimization
...
function minimize!(model; kwargs...)
    ...
    gradientbuf = copy(∇f)
    hessianbuf  = deepcopy(∇²f)
    fbuf        = 0.0
    last = copy(∇f)
    function calc_all(x)
        if x != last
            last .= x
            fbuf = calcall(hessianbuf, gradientbuf, x, model)
            apply!(hessianbuf, model.boundaryconds)
            apply_zero!(gradientbuf, model.boundaryconds)
        end
